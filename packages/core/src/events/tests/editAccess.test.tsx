...
describe('canvas edit access', () => {
  it('clears selection and hover through null-id canvas connectors', () => {
    const { store, handler } = setup();
    const canvas = document.createElement('div');
    const select = handler.handlers().select(canvas, null);
    const hover = handler.handlers().hover(canvas, null);
    act(() => {
      store.actions.selectNode('first');
      store.actions.setNodeEvent('hovered', 'first');
    });

    dispatch(canvas, 'mousedown');
    dispatch(canvas, 'click');
    dispatch(canvas, 'mouseover');
